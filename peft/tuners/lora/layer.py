...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        elif isinstance(init_lora_weights, str) and init_lora_weights == "sgdSvd":
            self.sgdSvd_init(adapter_name)
        elif isinstance(init_lora_weights, str) and init_lora_weights == "sgdSvd-alpha":
            self.sgdSvd_alpha_init(adapter_name)
        elif isinstance(init_lora_weights, str) and init_lora_weights == "sgdSvd-beta":
            self.sgdSvd_beta_init(adapter_name)
        elif isinstance(init_lora_weights, str) and init_lora_weights.startswith("milora"):
    ...
    def sgdSvd_init(self, adapter_name):
        weight = self.get_base_layer().weight
        dtype = weight.dtype
        lora_B, lora_A = self.sgd_svd(weight, adapter_name)
    ...
    #
    #     if distribution_type == 'kaiming_uniform':
    #         nn.init.kaiming_uniform_(self.lora_A[adapter_name].weight)
    #         nn.init.kaiming_uniform_(self.lora_B[adapter_name].weight)
    #     elif distribution_type == 'kaiming_normal':
    #         nn.init.kaiming_normal_(self.lora_A[adapter_name].weight)
    #         nn.init.kaiming_normal_(self.lora_B[adapter_name].weight)
    #     elif distribution_type == 'normal':
    #         nn.init.normal_(self.lora_A[adapter_name].weight,  mean=0.0, std=0.01)
    #         nn.init.normal_(self.lora_B[adapter_name].weight,  mean=0.0, std=0.01)
    #     elif distribution_type == 'uniform':
    #         nn.init.uniform_(self.lora_A[adapter_name].weight, a=-0.1, b=0.1)
    #         nn.init.uniform_(self.lora_B[adapter_name].weight, a=-0.1, b=0.1)
    #     elif distribution_type == 'orthogonal':
    #         nn.init.orthogonal_(self.lora_A[adapter_name].weight)
    #         nn.init.orthogonal_(self.lora_B[adapter_name].weight)
    #     elif distribution_type == 'sparsity':
    #         nn.init.sparse_(self.lora_A[adapter_name].weight, sparsity=0.5, std=0.01)
    #         nn.init.sparse_(self.lora_B[adapter_name].weight, sparsity=0.5, std=0.01)
    #     elif distribution_type == 'xavier_uniform':
    #         nn.init.xavier_uniform_(self.lora_A[adapter_name].weight)
    #         nn.init.xavier_uniform_(self.lora_B[adapter_name].weight)
    #     elif distribution_type == 'xavier_normal':
    ...
    #         nn.init.constant_(self.lora_A[adapter_name].weight, val=0.001)
    #         nn.init.constant_(self.lora_B[adapter_name].weight, val=0.001)
    #         print(self.lora_A[adapter_name].weight)
    #     else:
    #         raise Exception('not support weight distribution method')
    #
    #     weight = weight.data - self.scaling[adapter_name] * self.lora_B[adapter_name].weight.data @ self.lora_A[adapter_name].weight.data
    #     weight = weight.to(dtype)
    #     self.get_base_layer().weight.data = weight

    def sgd_svd(self, weight, adapter_name):
        print('##' * 50)

        root_path = '/work/Codes/IniLoRA/matrix_decomposition/init_weights/Llama-2-7b-hf/rank-8-iterNum-20000-lr-0.0005/'
        # root_path = '/work/Codes/IniLoRA/matrix_decomposition/init_weights/roberta-base/rank-8-iterNum-20000-lr-0.0005/'

        para_path = f'{self.kwargs["current_key"]}.weight.pt'
        para_path = re.sub('^roberta\.', '', para_path)
        assert os.path.exists(os.path.join(root_path, para_path))
        print(f'load sgd_svd weights from path: {os.path.join(root_path, para_path)}')

        UP = torch.load(os.path.join(root_path, para_path))
        return UP['A'], UP['B']

    def sgdSvd_alpha_init(self, adapter_name):
        print('##' * 50)
        print('is using IniLoRA-alpha Method')
        weight = self.get_base_layer().weight
        dtype = weight.dtype
        distribution_type = 'kaiming_normal'
        print(distribution_type)

        nn.init.normal_(self.lora_A[adapter_name].weight, mean=0, std=0.5)
        nn.init.normal_(self.lora_B[adapter_name].weight, mean=0, std=0.5)

        weight = weight.data - self.scaling[adapter_name] * self.lora_B[adapter_name].weight.data @ self.lora_A[adapter_name].weight.data
        weight = weight.to(dtype)
        self.get_base_layer().weight.data = weight

    def sgdSvd_beta_init(self, adapter_name):
        print('##' * 50)
        print('is using IniLoRA-beta Method')
        weight = self.get_base_layer().weight
        dtype = weight.dtype
        distribution_type = 'kaiming_normal'
        print(distribution_type)
