...
available_models = ['FacebookAI/roberta-base', 'FacebookAI/roberta-large']
...
if __name__ == '__main__':
    ...
    tasks = []
    for name, para in model.named_parameters():
        if any([e in name for e in elements]) and 'bias' not in name:
            module_name = 'query' if 'query' in name else 'value'
            tasks.append([name, para.data, module_name])
    print([n for n, _, _ in tasks])
