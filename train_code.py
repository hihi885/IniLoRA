...
class TrainingArguments(transformers.TrainingArguments):
    ...
    train_targets: str = field(
        default=None,
        metadata={"help": "True: train_targets"},
    )
...
#         assert instruction is not None
#         sources.append(instruction)
#     targets = [f"{output}{tokenizer.eos_token}" for output in examples['answer']]
#     # print('##' * 50)
#     # print(sources[:5])
#     # print(targets[:5])
#     # print('##' * 50)
...
def train():
    ...
    if script_args.init_lora_weights != 'fp':
        ...
        print('Warning: finetune all Linear lora')
    ...
    if script_args.init_lora_weights == 'fp' and script_args.train_targets:
        print(f'train targets: {script_args.train_targets}')
        for name, para in model.named_parameters():
            para.requires_grad_(False)
            if any([t in name for t in script_args.train_targets.split(',')]):
    ...
    # raw_train_datasets = load_dataset(script_args.data_path, split=f"train[:{script_args.data_length}]", cache_dir=script_args.cache_dir)
